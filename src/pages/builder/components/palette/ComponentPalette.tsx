...
import type { ComponentType, DroppableOptions, Node, TreeRoot } from "../../../../types/builder";
...
const TreeNode: Component<{ node: Node } & TreeSharedProps> = (props) => {
  ...
  const acceptsChildren = props.node.type === "container";
  let buttonRef: HTMLButtonElement | undefined;

  return (
    <li class="space-y-1" data-node-id={props.node.id}>
      <div
        class="rounded transition-all"
        use:droppable={{ id: props.node.id, onDrop: props.onDrop, acceptInside: acceptsChildren }}
      >
        <button
          ref={(el) => (buttonRef = el)}
          type="button"
          use:draggableNode={{ id: props.node.id, previewElement: () => buttonRef! }}
...
const TreeView: Component<{ root: TreeRoot } & TreeSharedProps> = (props) => (
  <div class="mt-6 rounded-md bg-white/5 p-3">
    ...
    <div
      data-node-id="root"
      class="mt-2 space-y-1 text-white/90 rounded"
      use:droppable={{ id: "root", onDrop: props.onDrop, acceptInside: true }}
    >
      <Show
        when={props.root.children.length > 0}
        fallback={<p class="text-xs text-white/60">Nenhum componente adicionado ainda.</p>}
      >
        <TreeBranch
